...
if( !$Tango::Debug && $Tango::Version > 0 && $Tango::Version >= 045 )
{
	return;
}

//Set the version number.
$Tango::Version = 045; // 0.4.5
...
//@deprecated
function GuiControl::getCenterPosition(%this)
{
	if(!isObject(%this)) return "0 0";
	return getWord(%this.extent,0)/2 SPC getWord(%this.extent,1)/2;
}

//Returns the center position of the object, does some math for the object it requests to center - Useful for only GuiControl::tangoMoveToCenter
//@param	GuiControl this
//@deprecated
function GuiControl::getCenterPosition_GUI(%this,%gui)
{
	if(!isObject(%this)) return "0 0";
	if(!isObject(%gui)) return "0 0";
	return getWord(%this.extent,0)/2 - getWord(%gui.extent,0)/2 SPC getWord(%this.extent,1)/2 - getWord(%gui.extent,1)/2;
}
...
//A function to move a GuiControl to a center position of the group it is attached to.
...
//This function does not support seamless movement or easing. It is an instantaneous modifier.
//@see	GuiControl::tangoMoveTo
function GuiControl::tangoMoveToCenter(%this, %ms, %easeModeX, %easeModeY)
{
	if(!isObject(%this)) return;
	if(!isObject(%group = %this.getGroup())) return;
	%this.tangoMoveTo(%group.getCenterPosition_GUI(%this),%ms,%easeModeX,%easeModeY);
}
